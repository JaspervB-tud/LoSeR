...
def test_evaluate_add_small_1():
    ...
    solution_object = solution.SolutionAverage(distances, clusters, selection, selection_cost=selection_cost, softmax_beta=beta, seed=1234)
    logsum_factor = solution_object.logsum_factor

    expected_candidate_objective, _ = calculate_objective(
        new_selection, distances, clusters, selection_cost, beta, logsum_factor
    )
    expected_numerators, expected_denominators = calculate_inter_components(
        new_selection, distances, clusters, beta, logsum_factor
    )
    expected_intra_changes = [
        (idx_to_add, idx_to_add, 0.0)
    ]
    expected_inter_changes = [
        (0, expected_numerators[0, 1], expected_denominators[0, 1]),
    ]
...
def test_evaluate_add_medium_1():
    ...
    expected_inter_changes = [
        (1, expected_numerators[0, 1], expected_denominators[0, 1]), 
    ]
...
def test_evaluate_add_large_1():
    ...
    expected_inter_changes = [
        (1, expected_numerators[0, 1], expected_denominators[0, 1]), 
        (2, expected_numerators[0, 2], expected_denominators[0, 2]), 
        (3, expected_numerators[0, 3], expected_denominators[0, 3]), 
    ]
...
def test_evaluate_add_large_2():
    ...
    expected_inter_changes = [
        (0, expected_numerators[3, 0], expected_denominators[3, 0]), 
        (1, expected_numerators[3, 1], expected_denominators[3, 1]), 
        (2, expected_numerators[3, 2], expected_denominators[3, 2]), 
    ]
...
def test_accept_add_small_1():
    ...
    expected_solution = solution.SolutionAverage(distances, clusters, new_selection, selection_cost=selection_cost, softmax_beta=beta, seed=1234)
...
def test_evaluate_swap_small_1():
    ...
    expected_inter_changes = [
        (0, expected_numerators[1, 0], expected_denominators[1, 0]),
    ]
...
def calculate_objective(selection, distances, clusters, cost_per_cluster, beta, logsum_factor):
    try:
    ...
    for cluster1, cluster2 in itertools.combinations(np.unique(clusters), 2):
        ...
        for idx1 in indices1:
            for idx2 in indices2:
                similarity = 1.0 - distances[idx1, idx2]
                numerator += similarity * math.exp(similarity * beta - logsum_factor)
                denominator += math.exp(similarity * beta - logsum_factor)
        components["inter"] += numerator / denominator
        objective += numerator / denominator

    return objective, components

def calculate_inter_components(selection, distances, clusters, beta, logsum_factor):
    numerators = np.zeros((len(np.unique(clusters)), len(np.unique(clusters))), dtype=np.float64)
    denominators = np.zeros((len(np.unique(clusters)), len(np.unique(clusters))), dtype=np.float64)
    ...
    for cluster1, cluster2 in itertools.combinations(np.unique(clusters), 2):
        ...
        numerator = 0.0
        denominator = 0.0
        for idx1, idx2 in itertools.product(indices1, indices2):
            similarity = 1.0 - distances[idx1, idx2]
            numerator += similarity * math.exp(similarity * beta - logsum_factor)
            denominator += math.exp(similarity * beta - logsum_factor)
        numerators[cluster1, cluster2] = numerator
        numerators[cluster2, cluster1] = numerator
        denominators[cluster1, cluster2] = denominator
        denominators[cluster2, cluster1] = denominator
...
def compare_inter_changes(actual_changes, expected_changes):
    assert len(actual_changes) == len(expected_changes)
    for (cluster, this_numerator, this_denominator), (expected_cluster, expected_numerator, expected_denominator) in zip(sorted(actual_changes), sorted(expected_changes)):
        assert cluster == expected_cluster
        np.testing.assert_array_almost_equal(this_numerator, expected_numerator, decimal=TOLERANCE)
        np.testing.assert_array_almost_equal(this_denominator, expected_denominator, decimal=TOLERANCE)